...
void uart_init(u32 bound){
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_USART1|RCC_APB2Periph_GPIOA, ENABLE);	//使能USARTOA时钟
...
#if EN_USART3   //
u8 USART3_RX_BUF[USART3_REC_LEN];     //????????,×??óUSART_REC_LEN??×???.
//????×???
//bit15??	?????ê??±ê??
//bit14??	??????0x0d
//bit13~0??	???????????§×???????
...
void USART3_Init(u32 BaudRate){ //USART3????????????
   ...
   RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB , ENABLE); //????UART3?ù??GPIOB???±??
   RCC_APB1PeriphClockCmd(RCC_APB1Periph_USART3, ENABLE); //??????????RCC?±??

   //??????????GPIO??????
   GPIO_InitStructure.GPIO_Pin = GPIO_Pin_11;//?è??USART3??RX??????PB11
   GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;//???????? ????????
   GPIO_Init(GPIOB, &GPIO_InitStructure);

   GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10;//?è??USART3??TX??????PB10
...
void USART3_IRQHandler(void){ 	
    ...
    if(USART_GetITStatus(USART3, USART_IT_RXNE) != RESET){  //????????
        ...
        if(Res=='S'){//??????????·???STOP?¨????????S??            
            USART3_RX_STA=1;//??????STOP?ò±ê??????1      
        }else if(Res=='K'){//??????????·???OK?¨????????K??            
            USART3_RX_STA=2;//??????OK?ò±ê??????2      
        }            
    }
}
#endif	
